...
Rectangle{
    ...
    states: [
        State {
            name: "unfold"
            PropertyChanges {target: fold;rotation: 0}
        },
        State {
            name: "fold"
            PropertyChanges {target: fold;rotation: 180}
        },
        State {
            name: "invalid"
            PropertyChanges {target: fold;visible: false}
        }
    ]
    TextInput{
    ...
    Image{
        ...
        MouseArea{
            anchors.fill: parent
            onClicked: root.state = (root.state == "unfold" ? "fold" : "unfold" )
        }
    }
